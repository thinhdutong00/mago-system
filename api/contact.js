...
export default async function handler(request, response) {
  ...
  const from = process.env.RESEND_FROM_EMAIL || 'Mago System <noreply@mago.digital.group>';
  const to = process.env.LEAD_TO_EMAIL || 'info@magodigital.it';
  ...
  if (!resendResponse.ok) {
    ...
    return response.status(502).json({
      error: details?.message || 'Invio email non riuscito.',
    });
  }
